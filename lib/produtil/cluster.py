...
class NOAAWCOSS(Cluster):
    ...
    def production(self):
        ...
        The return value may change during the execution of this
        program if a production switch happened.  In addition, the
        check requires opening and parsing the /etc/prod file, so the
        runtime is likely several milliseconds when the cache times
        out.  To force a refresh of this information, call
        self.uncache() first."""
        now=int(time.time())
